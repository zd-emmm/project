...
use Channel\Client;
use Channel\Server;
use tools\services\workerman\chat\ChatService;
use tools\services\workerman\WorkermanService;
...
class Workerman extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if(!$server || $server == 'admin'){
            var_dump('admin');
            Worker::$stdoutFile = $this->config['admin']['path'];
            //创建 admin 长连接服务
        ...
        if(!$server || $server == 'chat') {
            var_dump('chat');
            Worker::$stdoutFile = $this->config['chat']['path'];
            //创建 h5 chat 长连接服务
        ...
        if(!$server || $server == 'channel') {
            var_dump('channel');
            Worker::$stdoutFile = $this->config['channel']['path'];
            //创建内部通讯服务
